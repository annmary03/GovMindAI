...
sample_doc = news_collection.find_one()
  # Match your collection name from models.py
try:
...
def user_api(request):
    ...
    elif request.method == 'POST':  # NEW USER
        try:
    ...
    elif request.method == 'PUT':  # EDIT USER
        try:
            data = json.loads(request.body)
            ...
            # Check for conflicts (only if changing username/email)
            if 'username' in update_data or 'email' in update_data:
                ...
                if conflict:
                    return JsonResponse({"error": "Username or email already in use"}, status=400)
